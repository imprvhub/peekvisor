...
export const onRequest = defineMiddleware(
  async ({ cookies, locals, url, redirect }, next) => {
    ...
    const currentLang = langCodes.includes(langFromPath)
      ? langFromPath
      : defaultLang;
    ...
    try {
      ...
      if (token) {
        ...
        if (userInfo?.user) {
          ...
          if (
            userInfo.user.planExpiresAt &&
            userInfo.user.planExpiresAt < now
          ) {
            appLocals.userPlan = "basic";
    ...
    for (const restriction of planRestrictedRoutes) {
      if (
        pathname.includes(`/${restriction.route}`) &&
        appLocals.userPlan !== restriction.requiredPlan
      ) {
        return redirect(
          `/${currentLang}/dashboard?upgrade=${restriction.requiredPlan}`,
          302,
        );
      }
    }
